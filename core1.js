...
let state=loadState();
function loadState(){try{const x=JSON.parse(localStorage.getItem(STORAGE));if(x&&[1,2,3].includes(+x.version)){const m=mergeState(freshState(),x);m.version=+x.version;if(m.ui.autoAdvance==null)m.ui.autoAdvance=true;if(m.ui.compact==null)m.ui.compact=true;return m}}catch(e){}return freshState()}
...
let saveTimer;function save(){const el=document.getElementById('saveState');if(el)el.textContent='Saving';clearTimeout(saveTimer);try{localStorage.setItem(STORAGE,JSON.stringify(state))}catch(e){}if(el)el.textContent='Saved';if(typeof scheduleLivePublish==='function')scheduleLivePublish()}
