...
const SCORER_STORAGE='cbum-scorer-key-v2';
const LEGACY_SCORER_STORAGE='cbum-scorer-key';
...
async function verifyScorerKey(key){
  const r=await fetch(`${LIVE_BASE}/functions/v1/publish-live-score`,{method:'POST',headers:{'Content-Type':'application/json','apikey':LIVE_KEY,'x-scorer-key':key},body:JSON.stringify({event_id:LIVE_EVENT,action:'verify'})});
  if(!r.ok)throw new Error((await r.json().catch(()=>({}))).error||`HTTP ${r.status}`);
  return r.json();
}
async function fetchLiveRow(){
...
function hydrateRemote(row,initializeSpectator=true){
  if(!row?.payload?.version)return;
  const ui={...state.ui};state=mergeState(freshState(),row.payload);state.ui={...state.ui,...ui,holeResult:null};
  if(initializeSpectator&&!spectatorInitialized){const c=typeof activeSpectatorRound==='function'?activeSpectatorRound():null;if(c){state.ui.tab='rounds';state.ui.round=c;state.ui.hole=firstIncomplete(c)}spectatorInitialized=true}
...
function scheduleLivePublish(){
  ...
  liveTimer=setTimeout(async()=>{try{const out=await publishPayload(scorerKey(),sharedPayload());liveStatus='synced';liveLastRemoteAt=out?.updated_at||new Date().toISOString()}catch(e){liveStatus='error'}setLiveHeader()},450)
}
async function fetchLive(){
  try{
    const row=await fetchLiveRow();if(!row)return;
    liveLastRemoteAt=row.updated_at;liveStatus='synced';
    if(!isScorer()&&row.payload?.version){hydrateRemote(row,true);render()}
    setLiveHeader();
...
async function enableScorer(code){
  ...
  try{
    await verifyScorerKey(clean);
    const row=await fetchLiveRow();
    if(row?.payload?.version){hydrateRemote(row,false);liveLastRemoteAt=row.updated_at||liveLastRemoteAt}
    localStorage.setItem(SCORER_STORAGE,clean);
    localStorage.removeItem(LEGACY_SCORER_STORAGE);
    liveStatus='synced';spectatorInitialized=true;setLiveHeader();return true
  }catch(e){return false}
}
function disableScorer(){localStorage.removeItem(SCORER_STORAGE);localStorage.removeItem(LEGACY_SCORER_STORAGE);liveStatus='offline';spectatorInitialized=false;setLiveHeader();fetchLive()}
...
function renderLiveSettings(){return`<div class="card"><div class="eyebrow">Shared scoring</div><h2>${liveLabel()}</h2><p class="muted">${isScorer()?'This phone is the authorized scorer. Changes save locally first and publish automatically whenever service is available.':'This device is read-only. Enter the scorer setup code below to authorize this phone as the scorer.'}</p><div class="notice ${isScorer()?'note-green':''}">${isScorer()?(navigator.onLine?liveAge():'No service right now · scoring remains saved locally'):liveAge()}</div>${isScorer()?`<button class="btn secondary" style="margin-top:10px" data-scorer-disable>Leave scorer mode</button>`:`<div style="margin-top:10px"><label>Scorer setup code</label><div class="row"><input class="field" type="password" autocomplete="off" placeholder="CBUM-…" data-scorer-code><button class="btn" data-scorer-enable>Enable scoring</button></div><div class="tiny" style="margin-top:6px">Authorize one phone. All other devices remain read-only live viewers.</div></div>`}</div>`}
