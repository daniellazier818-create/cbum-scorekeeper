...
function renderLiveSettings(){return`<div class="card"><div class="eyebrow">Shared scoring</div><h2>${liveLabel()}</h2><p class="muted">${isScorer()?'This phone is the scoring authority. Changes save locally first and publish automatically whenever service is available.':'This device is read-only. It follows the scorer automatically and refreshes the shared tournament about every five seconds.'}</p><div class="notice ${isScorer()?'note-green':''}">${isScorer()?(navigator.onLine?liveAge():'No service right now · scoring remains saved locally'):liveAge()}</div>${isScorer()?`<button class="btn secondary" style="margin-top:10px" data-scorer-disable>Leave scorer mode</button>`:`<div style="margin-top:10px"><label>Scorer setup code</label><div class="row"><input class="field" type="password" autocomplete="off" placeholder="CBUM-…" data-scorer-code><button class="btn" data-scorer-enable>Enable scoring</button></div><div class="tiny" style="margin-top:6px">Authorize this phone once. After that, score and tee edits do not require repeated password entry.</div></div>`}</div>`}
