...
function renderLiveSettings(){return`<div class="card"><div class="eyebrow">Shared scoring</div><h2>${liveLabel()}</h2><p class="muted">${isScorer()?'This phone is authorized to edit scores, tees and handicaps. Changes save locally first and publish automatically whenever service is available.':'This device is read-only. Enter the scorer setup code below to authorize score and tee changes.'}</p><div class="notice ${isScorer()?'note-green':''}">${isScorer()?(navigator.onLine?liveAge():'No service right now · scoring remains saved locally'):liveAge()}</div>${isScorer()?`<button class="btn secondary" style="margin-top:10px" data-scorer-disable>Leave scorer mode</button>`:`<div style="margin-top:10px"><label>Scorer setup code</label><div class="row"><input class="field" type="password" autocomplete="off" placeholder="CBUM-…" data-scorer-code><button class="btn" data-scorer-enable>Enable editing</button></div><div class="tiny" style="margin-top:6px">Authorize this phone once. It will remain authorized until you leave scorer mode.</div></div>`}</div>`}
...
document.addEventListener('click',async e=>{
  ...
  if(on){const input=document.querySelector('[data-scorer-code]');on.disabled=true;on.textContent='Checking…';const ok=await enableScorer(input?.value);if(ok){toast('Editing enabled');render()}else{on.disabled=false;on.textContent='Enable editing';alert('That scorer setup code was not accepted.')}}
